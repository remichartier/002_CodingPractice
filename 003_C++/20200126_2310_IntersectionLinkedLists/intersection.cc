...
void testIntersection(){ // not yet compiled ...
	LinkedList<int> l1 = LinkedList<int>(nullptr);
	LinkedList<int> l2 = LinkedList<int>(nullptr);
	Node<int> * p1(l1.head);
	Node<int> * p2(l2.head);
	...
	auto posEnd = memorySet.end(); // Returns an iterator to the theoretical element that follows last element in the set.
	...
	while( (p2 != null) && (memorySet.find(p2) != posEnd ) ) {
		p2 = p2->next ;
	}
	...
	if (p2 == nullptr){
		...
		cout << "1 intersection found between the 2 linked list" << endl ;
	}
