...
bool found(vector<int> table, int n)
{
	if (!table.empty()) {
		for (int item:table){
...
int main(void)
{
	...
	for (int f:factors) {
		size_t i(0) ;
		int n = f * i ;
		cout << "multiples of " << f << " : " << endl ;
		while (n < MAX) {
			if (multiples.empty() || ! found(multiples,n)) {
...
g++ Multiples3And5_v01.cpp -o Multiples3And5_v01
remi@HPPortable:~/00_Mirrored/tocopyhomepc/docs/025_Travail/trainings/26_ProgramExercizing/003_C++/20190820_ProjectEuleurMultiplesOf3And5$ ./Multiples3And5_v01 bonjour
multiples of 3 : 
