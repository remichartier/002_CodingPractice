...
class Graph(object):
    ...
    def bfs(self, start_node_num):
        ...
        node.visited = True
        stack = []
        # 1st stage : visit edges, stack them in stack of node to handle later
        for edge in node.edges:
            if edge.node_from.value == node.value and not edge.node_to.visited:
                edge.node_to.visited = True
                ret_list.append(edge.node_to.value)
                stack.append(edge.node_to.value)
        #print(f'1st stage ret_list = {ret_list}, stack = {stack}')
        # Once over, unstack and redo based on unstacked node, until stack is empty.
        while len(stack) > 0:
            node_val = stack.pop(len(stack) -1)
            # go through all of its edges, if not yet visited, and stack them as well.
            node = self.find_node(node_val)
            for edge in node.edges:
                if edge.node_from.value == node_val and not edge.node_to.visited:
                    edge.node_to.visited = True
                    ret_list.append(edge.node_to.value)
                    stack.append(edge.node_to.value)
            #print(f'While ...: ret_list = {ret_list}, stack = {stack}')
...
# Stack : 'Shanghai', 'Berlin', 'Sao Paolo'
# ret : 'Shanghai', 'Berlin', 'Sao Paolo', 'Mountain View', 'San Francisco'
 

 # BUT I HAVE IT WRONG, BECAUSE BFS USUALLY SHOULD USE A QUEUE AND NOT A STACK ...
